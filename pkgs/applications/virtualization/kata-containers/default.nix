{ stdenv, buildGoPackage, fetchFromGitHub, fetchurl, pkgconfig
, libseccomp, yq-go, go, qemu }:
...
let
  ...
  agent = stdenv.mkDerivation {
    ...
    makeFlags = makeFlags ++ [ "UNIT_DIR=$(out)/lib/systemd/system" ];
  ...
  runtime = stdenv.mkDerivation {
    ...
    nativeBuildInputs = [ yq-go go ];

    inherit setSourceRoot makeFlags;

    patchPhase = ''
      substituteInPlace Makefile --replace '-i -o' '-o'
      substituteInPlace golang.mk --replace '$(GOPATH)/bin/yq' '${yq-go}/bin/yq'
    '';
